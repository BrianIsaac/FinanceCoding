"""Quick functional test for LSTM after fixes."""
...
def quick_test_lstm():
    """Run quick LSTM test on real data."""
    logger.info("=" * 80)
    logger.info("LSTM QUICK TEST")
    logger.info("=" * 80)
    ...
    config = LSTMModelConfig(use_markowitz_layer=False)  # Simpler for testing
    ...
    if all_passed:
        logger.info("✅ LSTM quick test PASSED")
        ...
        logger.error("❌ LSTM quick test FAILED")
        return False


if __name__ == "__main__":
    success = quick_test_lstm()
