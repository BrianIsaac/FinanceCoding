...
class SimplexProjectionHead(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            projection_method: Projection method ('softmax' or 'squared')
        """
        super().__init__()
    ...
    def forward(
        ...
        if mask is not None:
            # Set masked positions appropriately based on projection method
            if self.projection_method == "softmax":
                scaled_scores = scaled_scores.masked_fill(~mask, -1e9)
            else:  # squared method
                scaled_scores = scaled_scores.masked_fill(~mask, 0.0)

        # Project onto simplex using configured method
        if self.projection_method == "squared":
            # Paper formulation: w_u = s²_u / Σ_v s²_v
            # Use abs to handle negative scores, then square
            positive_scores = torch.abs(scaled_scores)
            squared_scores = positive_scores ** 2

            # Apply mask to squared scores
            if mask is not None:
                squared_scores = squared_scores * mask.float()

            # Normalise
            score_sum = squared_scores.sum(dim=-1, keepdim=True).clamp(min=1e-8)
            portfolio_weights = squared_scores / score_sum
        ...
        weight_sums = portfolio_weights.sum(dim=-1, keepdim=True).clamp(min=1e-8)
        portfolio_weights = portfolio_weights / weight_sums
...
class RelationAwareAllocationHead(SimplexProjectionHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__(input_dim, hidden_dim, num_layers, projection_method=projection_method, **kwargs)
