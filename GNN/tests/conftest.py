...
def sample_returns_data() -> pd.DataFrame:
    ...
    dates = pd.date_range(start=datetime(2023, 1, 1), periods=252, freq="B")  # Business days
    ...
    return pd.DataFrame(data=returns, index=dates, columns=assets)
...
def sample_config():
    ...
    """
    return {
        "data": {"lookback_days": 252, "min_observations": 100, "universe_size": 50},
        "model": {"hidden_dim": 64, "num_heads": 4, "dropout": 0.1, "learning_rate": 0.001},
        "training": {"batch_size": 16, "max_epochs": 10, "patience": 5},
        "gpu": {"max_memory_gb": 11.0, "enable_mixed_precision": True},
    }
...
def pytest_configure(config):
    ...
    if not torch.cuda.is_available():
        config.addinivalue_line("markers", "gpu: mark test as requiring GPU (skipped if no GPU)")
