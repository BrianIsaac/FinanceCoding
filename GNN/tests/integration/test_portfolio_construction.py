...
from src.evaluation.backtest.transaction_costs import TransactionCostConfig
from src.evaluation.metrics.returns import ReturnAnalyzer, ReturnMetricsConfig
from src.evaluation.reporting.export import PortfolioExporter, ExportConfig
...
class TestPortfolioConstructionIntegration:
    ...
    def test_end_to_end_equal_weight_portfolio(
        ...
        for i, weights_entry in enumerate(portfolio_weights_history):
            ...
            period_mask = (sample_returns_data.index >= period_start) & (sample_returns_data.index < period_end)
            ...
            if not period_returns.empty:
                ...
                if len(common_assets) > 0:
                    period_portfolio_returns = period_returns[common_assets].multiply(
                        weights[common_assets], axis=1
                    ).sum(axis=1)
    ...
    def test_transaction_cost_impact_integration(
        ...
        target_weights = model.predict_weights(
            sample_returns_data.index[150], sample_universe[:30]
        )
    ...
    def test_export_functionality_integration(self, temp_output_dir):
        ...
        with open(position_analysis_file, 'r') as f:
            position_data = json.load(f)
        
        assert len(position_data) > 0
        for asset, analysis in position_data.items():
...
from src.evaluation.backtest.transaction_costs import TransactionCostCalculator