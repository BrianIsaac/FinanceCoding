...
class TestConstraintEngine:
    ...
    def test_apply_basic_constraints_long_only(self, default_engine):
        ...
        negative_weights = pd.Series([-0.05, 0.15, 0.10, -0.02, 0.82], 
                                   index=[f"ASSET_{i:03d}" for i in range(5)])
        
        constrained = default_engine.apply_constraints(negative_weights)
        
        # All weights should be non-negative
        assert all(constrained >= 0), "All weights should be non-negative after constraint application"
    ...
    def test_apply_max_position_weight_constraint(self, strict_engine, sample_weights):
        ...
        assert new_max_weight < original_max_weight, "Max weight should be reduced after constraint application"
        
        # Weights should still sum to 1.0
        assert abs(constrained.sum() - 1.0) < 1e-10

    def test_apply_turnover_constraints_within_limit(self, default_engine, sample_weights, previous_weights):
    ...
    def test_apply_turnover_constraints_exceeds_limit(self, strict_engine, previous_weights):
        ...
        assert actual_turnover < original_turnover, "Turnover should be reduced by constraint engine"
    ...
    def test_calculate_constraint_metrics_with_turnover(self, default_engine, sample_weights, previous_weights):
    ...
    def test_risk_constraints_stub(self, default_engine, sample_weights):
        ...
        returns_data = pd.DataFrame(
            np.random.normal(0.001, 0.02, (100, len(sample_weights))),
        ...
        constrained = default_engine.apply_constraints(
            sample_weights, returns_data=returns_data
        )
