...
from src.models.gat.model import GATPortfolioModel, GATModelConfig, SharpeRatioLoss
...
class TestSharpeRatioLoss:
    ...
    def test_time_series_returns(self):
        ...
        weights = torch.tensor([[0.5, 0.3, 0.2]], dtype=torch.float32)  # [1, 3]
        returns = torch.tensor([[[0.1, 0.05, 0.02],
                                 [0.02, 0.03, 0.01],
                                 [-0.01, 0.02, 0.03],
                                 [0.03, -0.01, 0.02],
                                 [0.01, 0.04, -0.01]]], dtype=torch.float32)  # [1, 5, 3]
