...
class TestHRPClustering:
    ...
    def test_clustering_initialization(self):
        ...
        config = ClusteringConfig(
            linkage_method="complete", min_observations=200, correlation_method="spearman"
        )
    ...
    def test_correlation_distance_transformation(self, clustering_engine):
        ...
        correlation_matrix = pd.DataFrame(
            {"A": [1.0, 0.8, -0.2], "B": [0.8, 1.0, 0.5], "C": [-0.2, 0.5, 1.0]},
            index=["A", "B", "C"],
        )
    ...
    def test_invalid_linkage_method(self, clustering_engine, sample_returns_data):
        ...
        with pytest.raises(ValueError, match="Invalid linkage method"):
            clustering_engine.hierarchical_clustering(distance_matrix, linkage_method="invalid")
    ...
    def test_cluster_tree_leaf_nodes(self, clustering_engine):
        ...
        single_asset_returns = pd.DataFrame(np.random.randn(200, 1), columns=["SINGLE_ASSET"])
    ...
    def test_correlation_matrix_validation(self, clustering_engine):
        ...
        valid_corr = pd.DataFrame(
            {"A": [1.0, 0.5, 0.3], "B": [0.5, 1.0, -0.2], "C": [0.3, -0.2, 1.0]},
            index=["A", "B", "C"],
        )
    ...
    def test_min_correlation_threshold(self):
        ...
        config = ClusteringConfig(min_observations=100, min_correlation_threshold=0.3)
        clustering_engine = HRPClustering(config)

        # Create returns with known low correlations
        np.random.seed(42)
        returns = pd.DataFrame(np.random.randn(200, 3), columns=["A", "B", "C"])
    ...
    def test_quasi_diagonalization(self):
        ...
        config = ClusteringConfig(min_observations=100, enable_quasi_diagonalization=True)
        clustering_engine = HRPClustering(config)

        # Create sample correlation matrix and linkage
        np.random.seed(42)
        returns = pd.DataFrame(np.random.randn(200, 5), columns=["A", "B", "C", "D", "E"])
    ...
    def test_memory_efficient_correlation(self):
        ...
        config = ClusteringConfig(min_observations=50, memory_efficient=True, chunk_size=3)
        clustering_engine = HRPClustering(config)
        ...
        np.testing.assert_allclose(memory_efficient_corr.values, standard_corr.values, rtol=1e-10)
