...
class LSTMNetwork(nn.Module):
    ...
    def get_memory_usage(self, batch_size: int, sequence_length: int) -> int:
        """
        Estimate memory usage in bytes for given input dimensions.
...
class SharpeRatioLoss(nn.Module):
    ...
    def forward(
        ...
        # Compute Sharpe ratio (handle edge case of zero variance)
        mean_excess = excess_returns.mean()
        std_excess = excess_returns.std()

        if std_excess < 1e-8:
            # If variance is too small, return mean squared error instead
            return F.mse_loss(predicted_returns, actual_returns)
