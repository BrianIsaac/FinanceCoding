...
class ValidationBasedOptimizer:
    ...
    def _evaluate_single_split(self,
                              model: Any,
                              split: RollSplit,
                              data_timestamps: list[pd.Timestamp]) -> float:
        """
    ...
    def optimize(self,
                model_factory: Callable,
                data_timestamps: list[pd.Timestamp],
                study_name: str = "hyperopt_study") -> dict[str, Any]:
        """
...
class ModelHyperparameterSpaces:
    ...
    def get_unified_constraint_hyperparameter_space() -> list[HyperparameterSpace]:
        ...
        return [
            ...
            HyperparameterSpace(
                name='diversification_threshold',
                param_type='int',
                low=10,
                high=100
            )
    ...
    @staticmethod
    def get_reduced_space(model_type: str,
                         max_params: int = 10) -> list[HyperparameterSpace]:
        """
        ...
        priority_params = {
            'hrp': ['linkage_method', 'lookback_window', 'distance_metric',
                   'correlation_threshold', 'rebalance_frequency'],
            'lstm': ['hidden_size', 'num_layers', 'learning_rate', 'sequence_length',
                    'dropout_rate', 'batch_size', 'weight_decay'],
            'gat': ['hidden_dim', 'num_heads', 'learning_rate', 'num_layers',
                   'dropout_rate', 'batch_size', 'edge_threshold']
        }
...
def create_hrp_optimizer(validation_engine: RollingValidationEngine,
                        n_trials: int = 50,
                        reduced_space: bool = False) -> ValidationBasedOptimizer:
...
def create_lstm_optimizer(validation_engine: RollingValidationEngine,
                         n_trials: int = 100,
                         reduced_space: bool = False) -> ValidationBasedOptimizer:
...
def create_gat_optimizer(validation_engine: RollingValidationEngine,
                        n_trials: int = 80,
                        reduced_space: bool = False) -> ValidationBasedOptimizer:
...
class ValidationPerformanceTracker:
    ...
    def update(self,
              ...
              score: float,
              params: dict[str, Any],
              additional_metrics: dict[str, float] | None = None) -> bool:
        """
...
class EarlyStoppingCallback:
    ...
    def __call__(self, epoch: int, logs: dict[str, float]) -> bool:
        ...
        if current is None:
            logger.warning(f"Early stopping conditioned on metric '{self.monitor}' "
