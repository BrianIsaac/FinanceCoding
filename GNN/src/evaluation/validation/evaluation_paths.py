...
# local helpers
from src.evaluation.metrics.portfolio_metrics import compute_metrics_from_returns
    ...
    *,
    ...
    with torch.no_grad():
        ...
        for i, s in enumerate(samples_sorted):
            ...
            end_trading = (
                dates[-1]
                if i + 1 == len(samples_sorted)
                else dates[dates.searchsorted(samples_sorted[i + 1].ts, "left") - 1]
            )
    ...
    r_all = stitch_daily_returns(
        weights_by_window, window_dates, returns_daily, turnover_bps=turnover_bps
    )
    ...
    if out_dir is not None:
        ...
        if not r_all.empty:
            ...
            (1.0 + r_all).cumprod().rename("equity").to_frame().to_csv(
                out_dir / "topk_ew_equity_daily.csv"
            )
