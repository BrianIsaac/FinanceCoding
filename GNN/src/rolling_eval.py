...
from . import train as train_mod  # we will call train_mod.train_gat()
...
class RollSplit:
    ...
    def to_datestr_tuple(self) -> Tuple[str, str, str]:
        return (self.train_start.date().isoformat(),
...
def _clone_cfg_for_roll(base_cfg: DictConfig, split: RollSplit, seed: int, out_dir: Path) -> DictConfig:
...
def run_rolling(
    ...
    *,
    ...
    if not splits:
        raise RuntimeError("No rolling splits could be created; check your graph dates and parameters.")

    # Execute
    results: List[RollResult] = []
    for ridx, split in enumerate(splits):
        roll_dir = out_root / f"roll_{ridx:02d}_{split.train_start.date()}_{split.test_start.date()}"
        for seed in seeds:
    ...
    for rr in results:
        ...
        if rr.metrics_row is not None:
            ...
            rows.append({**base, "strategy": "GAT", "CAGR": np.nan, "AnnMean": np.nan, "AnnVol": np.nan, "Sharpe": np.nan, "MDD": np.nan})
    ...
    if r_all_concat:
        ...
        (1.0 + r_ens).cumprod().rename("equity").to_frame().to_csv(out_root / "ensemble_equity_daily.csv")
    ...
    return {"splits": [s.to_datestr_tuple() for s in splits], "rows": rows, "summary": summary_payload}
